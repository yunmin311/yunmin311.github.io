...
| 对话 | 职责 | 地盘 |
|------|------|------|
| **辅助对话** | 执行:代码、内容、环境、文档;**唯一的 git 提交者** | `design/` 以外的一切 |
| **设计辅助对话** | 视觉方向探索、设计稿(本地 HTML);**git 一概不碰** | `design/`(尚未建,首次产出时自建) |

拍板的人永远是用户;技术问题由辅助对话解答和转达。

## git 规矩(从 Creative OS 的教训里抄来的,别重新踩坑)

1. **只有辅助对话跑 git 写命令**;设计辅助对话连只读命令都用不着
